...
{\bf 3. Extract variable sequences.} For each variable in a program trace, the pipeline extracts the sequence of distinct values it takes on.
...
For example, in Student A's code and Student B's code, \codevar{i} and \codevar{k} take on the same sequence of values: $0$,$1$,$2$. They are therefore considered the same {\it common variable}. The following table shows the common and unique variables across Students A, B, and C.
\vspace{10mm}
\begin{tabular}{ll}
...
There are exceptions made to avoid name collisions. For example, in the running example, the unique variable's original name, \codevar{exp}, has a double underscore appended to it as a modifier to resolve a name collision with the common variable of the same name, referred to as a unique/common collision.
...
\begin{tabular}{ll}
{\bf Code with Multiple Instances} & {\bf Common Variable} \\
{\bf of a Common Variable} & {\bf Mappings} \\
\begin{minipage}{0.5\linewidth}
\begin{lstlisting}[basicstyle=\linespread{1.0}\ttfamily\footnotesize,language=python]
def iterPower(base,exp):
   result=1
