...
%DIF DEL ../pastMondaythesis/main.tex   Wed Aug 10 21:27:08 2016
%DIF ADD main.tex                       Wed Aug 10 21:27:11 2016
...
\DIFaddbegin \DIFadd{This chapter presents two workflows that prompt students to engage with the diversity of solutions written by fellow students, reflect on their own debugging successes, and write hints that may help current and future students. It is adapted from a paper presented at the ACM conference on
Computer-Supported Cooperative Work and
Social Computing (CSCW) in 2016~\mbox{%DIFAUXCMD
\cite{classoverflow}}%DIFAUXCMD
. References to ``the authors'' in this chapter refer to the coauthors of this paper.
}

\DIFaddend One-on-one human tutoring is a costly gold standard in education. As established in Bloom's seminal work on tutoring, mastery-based instruction with corrective feedback can offer a substantial improvement in learning outcomes over conventional classroom teaching \cite{bloom}. However, personalized support does not scale well with the number of students enrolled. In large classes, it is often not feasible for students to get personalized hints from a teacher in a timely manner. Massive open online courses (MOOCs) can have even worse teacher-to-student ratios, by orders of magnitude. Intelligent tutoring systems have strived to simulate the type of personalized support received in one-on-one tutoring, but they are expensive and time-consuming to build. 

\DIFdelbegin \DIFdel{In this chapter , students are asked to generate personalized hints for each other}\DIFdelend \DIFaddbegin \DIFadd{The two workflows in this chapter address this problem with a form of }{\it \DIFadd{learnersourcing}}\DIFaddend . Prior work on \DIFdelbegin \textit{\DIFdel{learnersourcing}} %DIFAUXCMD
\DIFdel{demonstrates how }\DIFdelend \DIFaddbegin \DIFadd{learnersourcing demonstrates that }\DIFaddend students can collectively generate educational content for future students, such as video outlines and exam questions, while engaging in a meaningful learning experience themselves \cite{kim2013learnersourcing,weir2015,mitros2015}. \DIFdelbegin \DIFdel{The proposed benefit of learnersourcing is that learners are not only more intrinsically motivated to engage with the learning content to begin with, but may also benefit pedagogically from the task itself.
}\DIFdelend %DIF > The proposed benefit of learnersourcing is that learners are not only more intrinsically motivated to engage with the learning content to begin with, but may also benefit pedagogically from the task itself.
...
This chapter \DIFdelbegin \DIFdel{describes }\DIFdelend \DIFaddbegin \DIFadd{presents }\DIFaddend two workflows for learnersourcing hints that assign hint-generating tasks to students based on what problems the student has recently solved. In the \textit{self-reflection} workflow, students generate hints by reflecting on an obstacle they themselves have recently overcome. In the \textit{comparison} workflow, students compare their own solutions to those of other students, generating a hint as a byproduct of explaining how one might get from one solution to the other. The second workflow can operate on the output of the first, as shown in Figure \ref{fig:workflow}. In both workflows, the key insight is that, through their own experience struggling with a particular problem, students can become experts on the particular optimizations they implement and bugs they resolve. The workflows can take pressure off teaching staff while giving students the valuable educational experiences of reflection and generating explanations. 

While such workflows could have many applications, this chapter \DIFdelbegin \DIFdel{presents a specific application within a }\DIFdelend \DIFaddbegin \DIFadd{describes their deployment in a }\DIFaddend college-level computer architecture \DIFdelbegin \DIFdel{class}\DIFdelend \DIFaddbegin \DIFadd{course}\DIFaddend . In this course, students implement, debug, and optimize simulated processors by constructing digital circuits. During both the debugging and optimization process, hints are one mechanism for teachers to help students fix and optimize their circuits. 
\DIFdelbegin \DIFdel{This paper applies our learnersourcing workflows }\DIFdelend \DIFaddbegin 

\DIFadd{The workflows are applied }\DIFaddend to two kinds of hints\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \textit{debugging hints} and \textit{optimization hints}. A debugging hint is a student's attempt to help a future student change their solution so it generates the expected output for a particular input. An optimization hint is a student's attempt to help a future student get from one correct solution to another, more optimal solution. When hint-receivers encounter that particular situation during their problem-solving process, the hints can be shown to them as if they are the personalized hints an intelligent tutoring system might generate, or that a teacher might provide during a one-on-one interaction. \DIFaddbegin \DIFadd{The system for learnersourcing debugging hints is called Dear Beta and the system for learnersourcing optimization hints is called Dear Gamma.
}\DIFaddend 

\section{System Design}
There are a number of necessary decisions to make when designing interventions to collect and deliver hints. This chapter provides some answers to key design questions in the context of two systems deployed within a computer architecture class\DIFdelbegin \DIFdel{: Dear Beta for learnersourcing debugging hints and Dear Gamma for learnersourcing optimization hints.}\DIFdelend \DIFaddbegin \DIFadd{.%DIF > : Dear Beta for learnersourcing debugging hints and Dear Gamma for learnersourcing optimization hints.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{During debugging, student solutions are run through sequences of teacher-designed }\DIFdelend \DIFaddbegin \DIFadd{The authors made specific indexing choices in order to build Dear Beta and Dear Gamma for students in the computer architecture course. In that course, students run their buggy solutions through teacher-designed sequences of }\DIFaddend test inputs. \DIFdelbegin \DIFdel{Because test cases have a specification of actual and expected outputs for each input, we decided to index }\DIFdelend \DIFaddbegin \DIFadd{Dear Beta indexes }\DIFaddend debugging hints by the \DIFdelbegin \DIFdel{tests for which the solution deviates from the expected output. In other words, debugging hints are associated with the }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend autograder error that disappears \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{once }\DIFaddend the bug is resolved. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend During optimization, the goal is not simply to attain a correct solution, but \DIFdelbegin \DIFdel{rather }\DIFdelend to arrive at a more optimal correct solution. \DIFdelbegin \DIFdel{We decided to index }\DIFdelend \DIFaddbegin \DIFadd{Dear Gamma indexes }\DIFaddend optimization hints by both start and end states\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{, which is }\DIFaddend the leap from a less optimal solution to a more optimal solution that the hint is intended to inspire. These states, the solutions themselves, are complex circuit objects. \DIFdelbegin \DIFdel{We use the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend number of transistors in a solution as a metric of its optimality. In this indexing scheme, all hints written with the intent of helping a student with a 114-transistor solution create a 96-transistor solution are binned together.
...
This ideal still leaves room for design choices about exactly which hints to deliver in a pull model of distribution. For example, during the optimization stage of an assignment, the system could \textit{always} give the student hints that help them create the \textit{next optimal solution} found by other students. This would hopefully be an optimization challenge that falls within the student's zone of proximal development \cite{ZMP}. However, this may be too cautious. If a student's solution is far less optimal than the most common solution, then the system could give the student hints to help them leap directly to that most common solution, without first creating any intermediate solutions. This strategy ignores how large a leap outside their zone of proximal development this might be, but it ensures that the student is exposed to the ideas, presented in those hints, that are necessary for implementing a solution at least as optimal as the most common one. \DIFdelbegin \DIFdel{We chose }\DIFdelend \DIFaddbegin \DIFadd{Dear Gamma uses }\DIFaddend this latter option, both hinting students toward the most common solution and hinting students with the most common solution toward the most optimal solution.

{\bf Should hint creation be a required task?} As discussed in Related Work, generating hints can be a valuable part of the learning process. \DIFdelbegin \DIFdel{We required all students }\DIFdelend \DIFaddbegin \DIFadd{All students were required }\DIFaddend to generate optimization hints as part of a reflection activity immediately after submitting their first correct circuit. \DIFdelbegin \DIFdel{We did not require students to generate debugging hints . This is because the number of bugsencountered could be large, and unlike optimizations, many bugs also may not lead to significant conceptual gain upon reflection.
Because debugging hints are immediately pushed out to all students, we keep hint creation voluntary to minimize the signal-to-noise ratio in hint quality.
}\DIFdelend \DIFaddbegin \DIFadd{In contrast, writing debugging hints was voluntary. Some bugs, like random typos, do not warrant required reflection.
}\DIFaddend 

{\bf How should the variation in hint quality be handled?} 
\DIFdelbegin \DIFdel{Allowing students to upvote hints they find helpful provides one signal of quality. When available, we used student upvotesto sort hints. When we required every student to independently generate hints and there was no affordance for upvoting helpful hints, we presented five }\DIFdelend \DIFaddbegin \DIFadd{When it is possible to collect data on helpfulness, collect it. For example, in a user interface, students can upvote hints and hints can be sorted by student upvotes. When }\DIFaddend hints \DIFaddbegin \DIFadd{are being pulled by students in need and there is little or no data about hint quality, the system could deliver multiple hints }\DIFaddend at a time\DIFdelbegin \DIFdel{to students in need of help}\DIFdelend . Even if one or more hints \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{in the delivered batch are }\DIFaddend of poor quality, their aggregate message might still be helpful for a student. If most of the hints \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{in the batch are }\DIFaddend about a feature that is irrelevant to the student receiving the hints, the remaining hint(s) might be about something different and more relevant. \DIFdelbegin \DIFdel{We limited each set of hints to a size of five to avoid overwhelming the student with too many hints}\DIFdelend \DIFaddbegin \DIFadd{Limiting the size of the batch to, for example, five, may prevent students from feeling overwhelmed}\DIFaddend .

{\bf How public should the hint-author be?} Many systems for question-answering have a reputation system, where the author is known and recognized for contributing answers. Previous work at CSCW has examined whether reputation would improve class forum participation\DIFaddbegin \DIFadd{~}\DIFaddend \cite{reputation}. For simplicity, \DIFdelbegin \DIFdel{we chose to leave student identitieshidden}\DIFdelend \DIFaddbegin \DIFadd{Dear Beta and Dear Gamma both hide student identities}\DIFaddend .
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.75\columnwidth]{Body/figures/classoverflow/CombinedWorkflow_CameraReady.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.75\columnwidth]{Body/figures/classoverflow/CombinedWorkflow_grey.pdf}
\DIFaddendFL \caption{In the \textit{self-reflection} workflow, students generate hints by reflecting on an obstacle they themselves have recently overcome. In the \textit{comparison} workflow, students compare their own solutions to those of other students, generating a hint as a byproduct of explaining how one might get from one solution to the other.}
\label{fig:workflow}
\end{figure}
%\todo{DNHT: Redo this figure}

\section{User Interfaces} 
\DIFdelbegin \DIFdel{We designed two user interfaces, one for each workflow. }\DIFdelend To learnersource debugging hints with the self-reflection workflow, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{the authors }\DIFaddend built and deployed {\it Dear Beta}, a Meteor web application that serves as a central repository of debugging advice for and by students in the class. The name alludes to both the ``Dear Abby'' advice column and the Beta processor that students create in the class. To learnersource optimization hints with the comparison workflow, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{the authors }\DIFaddend designed {\it Dear Gamma}, a web interface students were required to fill out as a reflection activity after submitting their final correct circuit for a class assignment.
\DIFaddbegin \DIFadd{Dear Beta and Dear Gamma each have their own user interface.
}\DIFaddend 

\subsection{Dear Beta}

\DIFdelbegin \DIFdel{We applied }\DIFdelend \DIFaddbegin \DIFadd{Dear Beta is an application of }\DIFaddend the self-reflection workflow to processor debugging. Consider students working on their Beta processors within the digital circuit development environment provided by the computer architecture class. Students run a staff-designed test file {\it x} on their circuit. The development environment alerts them to a autograder error: for a particular input (test number {\it n}), {\it y} was the expected output and the student's circuit returned {\it z}. 
...
\caption{{\it Dear Beta} serves as a central repository of debugging advice for and by students, indexed by autograder errors. In this figure, there are three learnersourced hints, sorted by upvotes, for a autograder error on test \DIFdelbeginFL \DIFdelFL{no. }\DIFdelendFL 33 in the `lab5/beta' checkoff file.}
\label{fig:hints}

\bigskip
\centering
\includegraphics[width=1.0\columnwidth]{Body/figures/classoverflow/contrib_shortened.png}
\caption{After fixing a bug, students can add a hint for others, addressing what mistake prevented their own solution from passing this particular \DIFdelbeginFL \DIFdelFL{verification }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{autograder }\DIFaddendFL test.}
\label{fig:contrib}
\end{figure}
...
After deploying initial prototypes of Dear Beta for two semesters, \DIFdelbegin \DIFdel{we invited Teaching Assistants }\DIFdelend \DIFaddbegin \DIFadd{Teaching Assistants were invited }\DIFaddend to share their complaints, requests, and experiences with \DIFdelbegin \DIFdel{us}\DIFdelend \DIFaddbegin \DIFadd{the authors}\DIFaddend . Four TAs were interviewed, in person or by email, and their feedback and experiences informed Dear Beta's final design.
...
\DIFdelbegin \DIFdel{In order to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend learnersource optimization hints, \DIFdelbegin \DIFdel{we caught students at a different stage in their learning process: }\DIFdelend \DIFaddbegin \DIFadd{students participate in the mandatory Dear Gamma hint writing activity }\DIFaddend right after they \DIFdelbegin \DIFdel{passed all verification }\DIFdelend \DIFaddbegin \DIFadd{first pass all autograder }\DIFaddend tests for a particular digital circuit, the Full Adder. \DIFdelbegin \DIFdel{Because students may have arrived at their solution without encountering any particular optimization obstacles, Dear Gamma uses the comparison workflow for learnersourcing rather than the self-reflection workflow. }\DIFdelend \DIFaddbegin \DIFadd{The student has reached correctness so they are not in a position to think about optimality. %DIF > Because students may have arrived at their solution without encountering any particular optimization obstacles, Dear Gamma uses the comparison workflow for learnersourcing rather than the self-reflection workflow. 
}\DIFaddend 
...
To evaluate the extent to which learnersourced hints can support problem solving, \DIFdelbegin \DIFdel{we deployed }\DIFdelend Dear Beta and Dear Gamma \DIFaddbegin \DIFadd{were deployed }\DIFaddend to the computer architecture class, which had an enrollment of more than two hundred students. Dear Beta was deployed for 6 weeks\DIFdelbegin \DIFdel{, during which we }\DIFdelend \DIFaddbegin \DIFadd{. During that time, the system }\DIFaddend collected student-generated debugging hints\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, while the authors and course staff }\DIFaddend observed the simultaneous usage of those hints \DIFdelbegin \DIFdel{in a real-world setting}\DIFdelend \DIFaddbegin \DIFadd{by students working on class assignments}\DIFaddend . The Dear Gamma optimization hint collection interface was released to students as \DIFaddbegin \DIFadd{a mandatory }\DIFaddend part of a particular lab. \DIFdelbegin \DIFdel{We then }\DIFdelend \DIFaddbegin \DIFadd{During a later offering of the same course, the authors }\DIFaddend conducted a lab study with nine students to understand how they solve a typical engineering problem using \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend learnersourced optimization hints. The questions our evaluation sought to answer are: (1) {\it What are the characteristics of student-generated hints?} and (2) {\it Can students solve problems using those hints?}

\subsection{Dear Beta}
The Dear Beta website was released as a stand-alone additional resource for students one week prior to the due date for the final circuit design lab. Students were made aware of its existence through a class forum announcement and signs on chalkboards in the course computer lab. It was left up for the remainder of the semester for students to refer to, if completing work late. \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{The system }\DIFaddend tracked student logins and engagement with site features. An initial prototype of Dear Beta was \DIFaddbegin \DIFadd{also }\DIFaddend deployed for two consecutive semesters prior to this final system design and study\DIFdelbegin \DIFdel{, as well}\DIFdelend .

\subsection{Dear Gamma}

\subsubsection{Hint Succession and Categorization}

While Dear Beta makes all hints available at all times, Dear Gamma is modeled on the hint-giving mechanism of an intelligent tutoring system. In prior work, sequences of hints have been posited to facilitate learning due to their similarity with sequences used in expert human tutoring, as well as their support of human memory processes \cite{sottilare2014design}. Therefore, \DIFdelbegin \DIFdel{we further }\DIFdelend \DIFaddbegin \DIFadd{the authors }\DIFaddend decomposed the hints collected with Dear Gamma into the three kinds of hints that typically comprise a hint sequence: 1) \textit{pointing hints} direct the student's attention to the location of error in case the student understood the general principle but did not know to apply it; 2) \textit{teaching hints} explain why a better solution exists by stating the relevant principle or concept; 3) \textit{bottom-out hints} indicate concretely what the student should do \cite{andes}. 
...
If one coder labeled a hint as a hybrid between two categories (i.e., teaching and pointing) while the other coder labeled it with only one category (i.e., pointing), \DIFdelbegin \DIFdel{we assigned the hint }\DIFdelend \DIFaddbegin \DIFadd{the hint was assigned }\DIFaddend to the pure category (i.e., pointing) that was in common between the two coders' labels. \DIFdelbegin \DIFdel{If there was no shared category across the two coders}\DIFdelend \DIFaddbegin \DIFadd{When the categories assigned by each coder did not overlap}\DIFaddend , the hint was discarded. \DIFdelbegin \DIFdel{We also excluded the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend minority of hints (3.2\%) \DIFdelbegin \DIFdel{that were }\DIFdelend labeled as irrelevant or unhelpful \DIFaddbegin \DIFadd{were excluded}\DIFaddend .

\subsubsection{Lab Study}

Nine out of the 226 current students in the computer architecture course participated in the study. These students were recruited through a course forum post \DIFdelbegin \DIFdel{. Participants were given }\DIFdelend \DIFaddbegin \DIFadd{and offered }\DIFaddend \$30 for \DIFdelbegin \DIFdel{the study, which lasted one hour. We informed students that we were studying the effectiveness of }\DIFdelend \DIFaddbegin \DIFadd{participating in one hour-long session. The author of this thesis ran all sessions as the experimenter. While obtaining informed consent from each student, the experimenter explained that the study would assess the effectiveness of previously collected }\DIFaddend hints for optimizing circuits\DIFdelbegin \DIFdel{so that they use fewer transistors.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{During the study, we presented the Dear Gamma hints , which were collected from }\DIFdelend \DIFaddbegin \DIFadd{, not the skills of the student. Students were not told that the hints were written by }\DIFaddend students in a previous semester \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{of the same class specifically for the Adder problem. Instead, the experimenter presented Dear Gamma hints }\DIFaddend as anonymous, potentially helpful messages. 
\DIFdelbegin \DIFdel{Three }\DIFdelend \DIFaddbegin 

\DIFadd{Students began the study by reviewing their solution to the Adder problem, which was due earlier in the term. All recruited students had completed it. During the study, three }\DIFaddend batches of hints were shown in the order of pointing, teaching, and bottom-out, but randomly selected within each category. The \DIFdelbegin \DIFdel{lab for which the Dear Gamma hints applied was due earlier in the semester and all recruited students had completed it. Students began the study by opening up their previously completed lab and reviewing their solution. The experimenter noted down }\DIFdelend \DIFaddbegin \DIFadd{experimenter noted }\DIFaddend the number of transistors in their solution, and randomly selected five pointing-type hints for a solution of that size from the Dear Gamma collection. For example, if the student had 114 transistors in their solution, they received five hints previously generated by students who had written a hint to help improve a 114-transistor solution. Because hints may be of variable quality, the researcher presented hints in batches of five to increase the chances that one of them might be helpful.
...
After receiving each batch of hints, participants answered the following 7-point Likert-scale questions about each hint (1: strongly disagree, 7: strongly agree): (1) {\it ``This hint taught me something.''} (2) {\it ``This hint helped me get to a more optimal circuit.''} and (3) {\it ``I feel more confident that I could solve a similar problem in the future.''} \DIFdelbegin \DIFdel{We placed these questions }\DIFdelend \DIFaddbegin \DIFadd{These questions were asked }\DIFaddend immediately after each batch of hints to capture user perception of hints at the time they occurred. However, to avoid slowing down the problem-solving process, participants were asked to explain their answers in writing only after the study, in the post-study questionnaire. This rating process was repeated for the teaching hints and bottom-out hints, even if students were able to solve the problem without asking for these hints.

After the study, users completed a post-study questionnaire regarding their overall impressions. Because users were shown a batch of hints at a time, all of which were student-generated, \DIFdelbegin \DIFdel{in }\DIFdelend the post-study questionnaire \DIFdelbegin \DIFdel{we added }\DIFdelend \DIFaddbegin \DIFadd{included }\DIFaddend additional Likert-scale items, ``I was able to find the most helpful hints and ignore the rest'' and ``Many hints felt repetitive,'' to understand whether users felt they could adequately ignore irrelevant hints. 

\subsection{Limitations}
These studies do not have control groups, so \DIFdelbegin \DIFdel{we cannot make }\DIFdelend \DIFaddbegin \DIFadd{the results cannot support }\DIFaddend any claims about student learning. \DIFdelbegin \DIFdel{We can only report }\DIFdelend \DIFaddbegin \DIFadd{Dear Beta was deployed in a real classroom setting while students worked on a relevant class assignment, and the results contain }\DIFaddend qualitative and quantitative measures of student engagement with the system \DIFaddbegin \DIFadd{only}\DIFaddend . Some of those observed behaviors and opinions may be derived from the participants' sense of novelty, rather than the underlying value of the system. \DIFdelbegin \DIFdel{We deployed Dear Beta in a real classroom setting, and in the context of a real assignment, for the purpose of observing natural interaction with the system. 
}\DIFdelend %DIF > For the purpose of observing natural interactions with the system, Dear Beta was deployed in a real classroom setting while students worked on a relevant class assignment. 
...
While working through their optimizations and hints, $S_6$ was the one student who significantly deviated from the correct line of thought by removing all inverting gates. Optimal solutions have \DIFdelbegin %DIFDELCMD < {\it %%%
\DIFdel{only }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{only }\DIFaddend inverting gates. As soon as $S_6$ saw that their transistor count had increased rather than decreased, they revisited the hints, realized their mistake, and correctly optimized their circuit. None of the hints themselves were incorrect, though some were deemed irrelevant or unhelpful.
...
\section{Discussion}
In this section, \DIFdelbegin \DIFdel{we first address }\DIFdelend the research questions \DIFdelbegin \DIFdel{the evaluation was intended to answer. We then explain that, of the design decisions made during the design of Dear Beta and Dear Gamma, the critical factors for success included }\DIFdelend \DIFaddbegin \DIFadd{are revisited first. Then critical design decisions are reviewed, including }\DIFaddend prompt clarity, \DIFdelbegin \DIFdel{the index chosen }\DIFdelend \DIFaddbegin \DIFadd{index choice }\DIFaddend for hints, \DIFdelbegin \DIFdel{how alternative solutionsare represented, and the use of }\DIFdelend \DIFaddbegin \DIFadd{representation choice for alternative solutions, and }\DIFaddend hint progressions.

\subsection{Answers to Research Questions}

Our study sought to evaluate the characteristics of student-generated hints. \DIFdelbegin \DIFdel{We can see from Table \ref{tab:hintTypes} }\DIFdelend \DIFaddbegin \DIFadd{Table \ref{tab:hintTypes} shows }\DIFaddend that students, without coaching, can naturally produce hints that point, teach, give a bottom-out direction, or provide some combination of those elements. However, the number of pointing hints labeled by {\it both} coders as purely pointing-type (22) was much smaller than the number of such hints in the teaching (75) and bottom-out (64) categories. Because students were not informed that their hint would be slotted into a progression, it is possible they may have felt that if they were going to give a future student one hint, it would need to be more substantial than just pointing to a particular location in the solution and hoping the hint-receiver would see the possibility of optimization.
...
The success of the comparison workflow depends not only on the index for solutions, but also on how these solutions are represented in the workflow prompt for hints. In the comparison workflow, \DIFdelbegin \DIFdel{we found that transistor counts sometimes did not account for }\DIFdelend \DIFaddbegin \DIFadd{transistor counts did not always capture }\DIFaddend lower-level reasons for a suboptimal circuit\DIFdelbegin \DIFdel{, resulting }\DIFdelend \DIFaddbegin \DIFadd{. This resulted }\DIFaddend in hints that were unhelpful for solutions with lower-level suboptimality. Students will not generate hints that account for what has been abstracted away in the representation of solutions in the hint prompt. Likewise, if the definition of optimality used to index solutions does not account for a certain kind of suboptimality, the hints generated will be unlikely to help future students with that kind of suboptimality. 

Lastly, when students request hints as they did in the Dear Gamma lab study, conforming to the intelligent tutoring system model of providing progressively specific hints is recommended. To automatically create hint progressions in the future, \DIFdelbegin \DIFdel{we could apply }\DIFdelend machine learning methods \DIFdelbegin \DIFdel{to estimate }\DIFdelend \DIFaddbegin \DIFadd{could classify }\DIFaddend a given hint \DIFdelbegin \DIFdel{'s }\DIFdelend type. Alternatively, \DIFdelbegin \DIFdel{we could learnersource hint classification}\DIFdelend \DIFaddbegin \DIFadd{hint type classification could be learnersourced}\DIFaddend . 

\subsection{Generalization}
Although \DIFdelbegin \DIFdel{we applied these workflows }\DIFdelend \DIFaddbegin \DIFadd{these workflows were applied }\DIFaddend to computer architecture problems, the self-reflection and comparison workflows could be extended to other domains. The workflows can be most readily applied to solutions that can be objectively tested for satisfying a set of requirements, e.g., passing unit tests, or whose optimality can be objectively measured. In domains without objective test cases or definitions for optimality, it may be more difficult to establish indices for clustering hints. In these cases, students could be asked to write what challenge they overcame or select from a growing list, enabling others to search for those terms. The comparison workflow could be modified to simply pair students with solutions different than their own, letting them judge for themselves which they think is better, the alternative solution or their own, and write hints based on that judgment.
