...
%DIF DEL ../pastMondaythesis/main.tex   Wed Aug 10 21:08:26 2016
%DIF ADD main.tex                       Wed Aug 10 21:08:14 2016
...
\DIFdelbegin %DIFDELCMD < \todo{Rob: it would be great to show example code for these tasks, ideally showing them in OverCode -- can you mine any examples from your thesis defense talk?}
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo{DNHT: Rob: it would be great to show example code for these tasks, ideally showing them in OverCode -- can you mine any examples from your thesis defense talk?}
...
The OverCode user interface is the product of an iterative design process with multiple stages, including paper prototypes and low-fidelity prototypes rendered in a web browser. \DIFdelbegin \DIFdel{Prototype iterations were used and critiqued by members of our research group and by several teaching staff of an introductory Python programming course}\DIFdelend \DIFaddbegin \DIFadd{Experts in user interface design and programming teachers critiqued initial prototypes}\DIFaddend . While exploring the low-fidelity prototypes, \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend teachers talked aloud about their hopes for what the tool could do, frustrations with its current form, and their frustrations with existing solution-viewing tools and processes. This feedback was incorporated into the final design.
...
\DIFdelbegin %DIFDELCMD < \todo{ROB:maybe the TOCHI paper didn't have space for screenshots of these, but your thesis needs them}
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo{ROB:maybe the TOCHI paper didn't have space for screenshots of these, but your thesis needs them}

%DIF >  \begin{figure}
%DIF >  \includegraphics[width=0.5\paperwidth]{Body/figures/overcode/rewrite_rule.jpg}
%DIF >  \caption{UI for adding rewrite rules.}
%DIF >  \label{fig:rewriterule}
%DIF >  \end{figure}
\DIFaddbegin 

\DIFaddend \subsubsection{Stacks} A stack in OverCode denotes a set of similar solutions that are grouped together based on a similarity criterion defined in Section \ref{pipeline}. For example, a stack for the \codevar{iterPower} problem is shown in Figure~\ref{stacks}(a). The size of each stack, i.e., the number of solutions in a stack, is shown in a pillbox at the top-left corner of the stack. Stacks are listed in the scrollable second panel from largest to smallest. The solution on the top of the stack is a platonic solution that describes all the solutions in the stack. See Section \ref{pipeline} for details on the normalizing process that produces platonic solutions.

Each stack can also be clicked. After clicking a stack, the border color of the stack changes and the \emph{done} progress bar is updated to reflect the percentage of total solutions clicked, as shown in Figure~\ref{stacks}(b). This feature is intended to help teachers remember which stacks they have already read or analyzed, and keep track of their progress. Clicking on a large stack, which represents a significant fraction of the total solutions, is reflected by a large change in the \emph{done} progress bar. \DIFdelbegin %DIFDELCMD < \todo{add that now the raw solutions inside the stack are also displayed, but this was not included during user study tests}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Double clicking on any stack reveals the raw solutions that belong to that stack. This last feature was added after user testing.%DIF >  \todo{add that now the raw solutions inside the stack are also displayed, but this was not included during user study tests}
}\DIFaddend 
...
{\bf 2. Execute solutions.} Each solution is on the same test case(s). During each step of the execution for each test case, the names and values of local and global variables, and also return values from functions, are recorded as a program trace \DIFdelbegin \DIFdel{using an adaptation of Philip Guo's logger}\DIFdelend \DIFaddbegin \DIFadd{adapted from the execution logger written by Philip Guo}\DIFaddend ~\cite{pgbovineOPT}. There is one program trace per solution per test case. Included for the purpose of illustrating this pipeline, the examples in the figures that follow are derived from executing definitions of \codevar{iterPower} on a \codevar{base} of $5.0$ and an \codevar{exp} of $3$, as illustrated in Figure~\ref{fig:testcase}.
\begin{figure}
\begin{tabular}{l}
{\bf Student \DIFdelbeginFL \DIFdelFL{Code }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Solution }\DIFaddendFL with Test Case} \\
\begin{minipage}{0.5\linewidth}
\begin{lstlisting}[basicstyle=\linespread{1.0}\ttfamily\footnotesize,language=python]
def iterPower(base,exp):
    #student \DIFdelbeginFL \DIFdelFL{code }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{solution }\DIFaddendFL here
iterPower(5.0, 3)
\end{lstlisting}
\end{minipage} 
\end{tabular}
...
{\bf 4. Identify common variables.} The pipeline analyzes all program traces, identifying which \DIFdelbegin \DIFdel{variables' }\DIFdelend \DIFaddbegin \DIFadd{variable value }\DIFaddend sequences are identical. Variables that have identical sequences \DIFaddbegin \DIFadd{of values }\DIFaddend across two or more program traces are referred to as {\it common variables}. Variables which occur in only one program trace are called {\it unique variables}. This is illustrated in Figures~\ref{fig:commik} and~\ref{fig:allvars}.
\begin{figure}
\begin{tabular}{ll}
{\bf Student B \DIFdelbeginFL \DIFdelFL{Code }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Solution }\DIFaddendFL with Test Case} & {\bf Variable Sequences for Student B \DIFdelbeginFL \DIFdelFL{Code}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Solution}\DIFaddendFL } \\
\begin{minipage}{0.5\linewidth}
\begin{lstlisting}[basicstyle=\linespread{1.0}\ttfamily\footnotesize,language=python]
def iterPower(base,exp):
    r=1
...
\end{lstlisting}
\end{minipage} \\
{\bf Student C \DIFdelbeginFL \DIFdelFL{Code }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Solution }\DIFaddendFL with Test Case} & {\bf Variable Sequences for Student C \DIFdelbeginFL \DIFdelFL{Code}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Solution}\DIFaddendFL } \\
\begin{minipage}{0.5\linewidth}
\begin{lstlisting}[basicstyle=\linespread{1.0}\ttfamily\footnotesize,language=python]
def iterPower(base,exp):
   result=1
...
\end{minipage}
\end{tabular}
\caption{\codevar{i} and \codevar{k} take on the same sequence of values\DIFaddbeginFL \DIFaddFL{, $0$ then $1$ then $2$, }\DIFaddendFL in \DIFaddbeginFL \DIFaddFL{the solutions of }\DIFaddendFL Student A \DIFdelbeginFL \DIFdelFL{'s }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{Student }\DIFdelendFL B\DIFdelbeginFL \DIFdelFL{'s code: $0$,$1$,$2$}\DIFdelendFL . They are therefore considered the same {\it common variable}.}
\label{fig:commik}
\end{figure}
\begin{figure}
{\bf Common variables} in \DIFaddbeginFL \DIFaddFL{solutions of }\DIFaddendFL Students A, B, and C\DIFdelbeginFL \DIFdelFL{'s code}\DIFdelendFL :
 \begin{itemize} 
\item \texttt{5.0}:
 \begin{itemize} 
...
\item \texttt{k} (Student B)
 \end{itemize} 
 \end{itemize} 
{\bf Unique variables} in \DIFaddbeginFL \DIFaddFL{solutions of }\DIFaddendFL Students A, B, and C\DIFdelbeginFL \DIFdelFL{'s code}\DIFdelendFL :
 \begin{itemize} 
\item \texttt{3,2,1,0}:
 \begin{itemize} 
...
For example, a subset of \codevar{iterPower} solutions created a variable that iterated through the values generated by \codevar{range(exp)}. \DIFdelbegin \DIFdel{Student A's code }\DIFdelend \DIFaddbegin \DIFadd{The Student A solution }\DIFaddend is an example. A smaller subset created a variable that iterated through the values generated by \codevar{range(1,exp+1)}, as seen in \DIFdelbegin \DIFdel{Student E's code}\DIFdelend \DIFaddbegin \DIFadd{the Student E solution}\DIFaddend . These are two separate common variables in our pipeline, due to their differing value sequences. The {\it common/common} name collision arises because both common variables are most frequently named \codevar{i} across all solutions to \codevar{iterPower}. To preserve the one-to-one mapping of variable name to value sequence across the entire \codevar{iterPower} problem dataset, the pipeline appends a modifier, \codevar{B}, to the common variable \codevar{i} found in fewer \codevar{iterPower} solutions. A common variable, also most commonly named \codevar{i}, which is found in even fewer \codevar{iterPower} definitions, will have a \codevar{C} appended, etc. This is illustrated in Figure~\ref{fig:commcommcoll}.
\begin{figure}
\begin{tabular}{l|l}
...
{\bf Multiple-instances collision.} The pipeline identifies variables by their sequence of values\DIFdelbegin \DIFdel{(excluding consecutive duplicates), not by their given name in any particular solution. However, without considering }\DIFdelend \DIFaddbegin \DIFadd{. This representation does not preserve information about }\DIFaddend the timing of \DIFdelbegin \DIFdel{variables' transitions between values}\DIFdelend \DIFaddbegin \DIFadd{when one value transitions to the next}\DIFaddend , relative to \DIFdelbegin \DIFdel{other variables in scope at each step of a function execution}\DIFdelend \DIFaddbegin \DIFadd{the values of other variables. Without those relative transition times}\DIFaddend , it is not possible to differentiate between multiple instances of a common variable within a single solution \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{or identically behaving, semantically distinct Boolean flags in different solutions. This is not a fundamental limitation of this method, because this information is in the program trace. Future versions of OverCode could extract it as well.
}\DIFaddend 

Rather than injecting a name collision into an otherwise correct solution, the pipeline preserves the \DIFdelbegin \DIFdel{solution author's variable name choice }\DIFdelend \DIFaddbegin \DIFadd{variable name chosen by the student }\DIFaddend for all the instances of that common variable in that solution. If \DIFdelbegin \DIFdel{a solution author's preserved variable }\DIFdelend \DIFaddbegin \DIFadd{the student-chosen }\DIFaddend name collides with any common variable name in any program trace and does not share \DIFdelbegin \DIFdel{that common variable's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend sequence of values, the pipeline appends a double underscore to the \DIFdelbegin \DIFdel{solution author's preserved variable }\DIFdelend \DIFaddbegin \DIFadd{student-chosen }\DIFaddend name, so that \DIFdelbegin \DIFdel{the interface, }\DIFdelend \DIFaddbegin \DIFadd{both the interface }\DIFaddend and the human reader \DIFdelbegin \DIFdel{, }\DIFdelend do not conflate them.
...
For evaluating both the analysis pipeline and the user interface of OverCode, the dataset of solutions is collected from 6.00x, an introductory programming course in Python that was offered on edX in fall 2012. Three \DIFdelbegin \DIFdel{exercise }\DIFdelend \DIFaddbegin \DIFadd{programming }\DIFaddend problems were chosen. This dataset consists of student solutions submitted within two weeks of \DIFdelbegin \DIFdel{each problem's release date }\DIFdelend \DIFaddbegin \DIFadd{the release date of each problem}\DIFaddend . There are thousands of solutions to these problems in the dataset\DIFdelbegin \DIFdel{, from which all correct solutions , tested over a set of test cases, were selected }\DIFdelend \DIFaddbegin \DIFadd{. All solutions marked as correct by an autograder were passed on to OverCode }\DIFaddend for analysis. The number of solutions analyzed for each problem is shown in Figure~\ref{overcode_solutioncounttable}.
...
\item {\bf \codevar{hangman}} The \codevar{hangman} problem takes a string \codevar{secretWord} and a list of characters \codevar{lettersGuessed} as input, and asks students to write a function that returns a string where all letters in \codevar{secretWord} that are not present in the list \codevar{lettersGuessed} are replaced with an underscore. This was a part of the third week of problem set \DIFdelbegin \DIFdel{exercises}\DIFdelend \DIFaddbegin \DIFadd{problems}\DIFaddend . See Figure \ref{hmexamples} for examples.
\item {\bf \codevar{compDeriv}} The \codevar{compDeriv} problem requires students to write a Python function to compute the derivative of a polynomial, where the coefficients of the polynomial are represented as a Python list. This was also a part of the third week of problem set \DIFdelbegin \DIFdel{exercises}\DIFdelend \DIFaddbegin \DIFadd{problems}\DIFaddend . See Figure \ref{cdexamples} for examples.
 \end{itemize} 

The three \DIFdelbegin \DIFdel{exercises were selected for analysis because they are representative of the typical exercises }\DIFdelend \DIFaddbegin \DIFadd{programming problems represent the typical problems }\DIFaddend students solve in the early weeks of an introductory programming course. The three \DIFdelbegin \DIFdel{exercises }\DIFdelend \DIFaddbegin \DIFadd{problems }\DIFaddend have varying levels of complexity and ask students to perform loop computation over three fundamental Python data types, integers (\codevar{iterPower}), strings (\codevar{hangman}), and lists (\codevar{compDeriv}). The \DIFdelbegin \DIFdel{exercises }\DIFdelend \DIFaddbegin \DIFadd{problems }\DIFaddend span the second and third weeks of the course in which they were assigned.
...
The first study was a 12-person within-subjects evaluation of interface satisfaction when using OverCode for a realistic, relatively unstructured task. Using either OverCode or a baseline interface, subjects \DIFdelbegin \DIFdel{were asked to }\DIFdelend browse student solutions to the programming problems in our dataset and then write a class forum post on the good and bad ways students solved the problem. \DIFdelbegin \DIFdel{Through this study , the first hypothesis is tested}\DIFdelend \DIFaddbegin \DIFadd{This study tests the hypothesis about interface satisfaction (}\textbf{\DIFadd{H1}}\DIFadd{)}\DIFaddend .

\subsubsection{OverCode and Baseline Interfaces}

\DIFdelbegin \DIFdel{Two interfaceswere designed}\DIFdelend \DIFaddbegin \DIFadd{The experiment relies on two interfaces}\DIFaddend , referred to here as OverCode and the baseline. The OverCode interface and backend are described in detail in Section \ref{overcode}. The baseline interface \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend a single webpage with all student solutions concatenated in a random order into a flat list\DIFdelbegin \DIFdel{(Figure \ref{iterPowerEdXControl}, left)}\DIFdelend \DIFaddbegin \DIFadd{, as shown in Figure \ref{iterPowerEdXControl}}\DIFaddend . This design emulates existing methods of reviewing solutions, and aims to draw out differences between browsing stacked and unstacked solutions. This is analogous to the \DIFdelbegin \DIFdel{``flat '' }\DIFdelend \DIFaddbegin \DIFadd{flat }\DIFaddend interface chosen as a baseline \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in another study on interfaces for grading clusters of student work published by }\DIFaddend Basu et al.\DIFdelbegin \DIFdel{'s interface for grading clusters of short answers}\DIFdelend ~\cite{basuDivideAndConquer}. Basu et al. \DIFdelbegin \DIFdel{'s assumption, }\DIFdelend \DIFaddbegin \DIFadd{assume }\DIFaddend that existing options for reviewing solutions are limited to going through solutions one-by-one\DIFdelbegin \DIFdel{, is backed }\DIFdelend \DIFaddbegin \DIFadd{. This assume is supported }\DIFaddend by our pilot studies\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend interviews with teaching staff, and our own grading experiences. In \DIFdelbegin \DIFdel{fact, in }\DIFdelend edX programming MOOCs, teachers are not even provided with an interface for viewing all solutions at once. They can only look at one student solution at a time. If the solutions can be downloaded locally, some teachers may use a search tool like \codevar{grep}. Our baseline allows for search too, through the in-browser \texttt{Find} command. 

\DIFdelbegin \DIFdel{In the baseline, solutions appeared visually similar to those in the }\DIFdelend \DIFaddbegin \DIFadd{Each solutions is rendered with the same syntax highlighting, background color, and borders in the baseline and OverCode interfaces. However, the solutions shown in the baseline are raw, and the solutions shown in }\DIFaddend OverCode \DIFdelbegin \DIFdel{interface (boxed, syntax-highlighted code), but the solutions were raw, in the sense that they were not normalized for whitespace or variable naming differences. As in the OverCode condition, subjects were able to }\DIFdelend \DIFaddbegin \DIFadd{are normalized. In both interfaces, subjects can }\DIFaddend use standard web-browser features, such as the within-page \texttt{Find} action.
...
\end{tabular}
\caption{Screenshots of the baseline interface. The appearance \DIFdelbeginFL \DIFdelFL{did not change significantly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{changed }\DIFaddendFL between the Forum Post study \DIFdelbeginFL \DIFdelFL{control interface, shown on the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL left\DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL and the Coverage study \DIFdelbeginFL \DIFdelFL{control interface, shown on the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL right\DIFaddbeginFL \DIFaddFL{) in order to minimize superficial differences between the baseline and OverCode interfaces}\DIFaddendFL . Functionality did not change\DIFdelbeginFL \DIFdelFL{at all}\DIFdelendFL .}
\label{iterPowerEdXControl}
\end{figure}

\subsubsection{Participants}

Participants \DIFdelbegin \DIFdel{were recruited by reaching out to }\DIFdelend \DIFaddbegin \DIFadd{joined the study by responding to advertisements sent out to an MIT CSAIL-wide email list as well as }\DIFaddend past and present \DIFdelbegin \DIFdel{programming course staff at our university, and advertising on an MIT CSAIL email list}\DIFdelend \DIFaddbegin \DIFadd{staff members of MIT introductory programming courses}\DIFaddend . These individuals were qualified to participate \DIFdelbegin \DIFdel{in our study because they met }\DIFdelend \DIFaddbegin \DIFadd{if they have }\DIFaddend at least one of the following requirements: (1) \DIFdelbegin \DIFdel{have }\DIFdelend taught a computer science course (2) \DIFdelbegin \DIFdel{have }\DIFdelend graded Python code before, or (3) \DIFdelbegin \DIFdel{have }\DIFdelend significant Python programming experience, making them potential future teaching staff. \DIFdelbegin \DIFdel{Subjects were offered }\DIFdelend \DIFaddbegin \DIFadd{Participants received }\DIFaddend \$20 \DIFdelbegin \DIFdel{to participate}\DIFdelend \DIFaddbegin \DIFadd{for an hour of their time in the lab}\DIFaddend . 

\DIFdelbegin \DIFdel{Information about the subjects' backgrounds was collected }\DIFdelend \DIFaddbegin \DIFadd{Subjects filled out forms about themselves }\DIFaddend during recruitment and again at the beginning of their one-hour in-lab \DIFdelbegin \DIFdel{user study }\DIFdelend session. 12 people (7 male) participated, with a mean age of 23.5 ($\sigma = 3.8$). Subjects had a mean 4 years of Python programming experience ($\sigma = 1.8$), and $75\%$ of participants had graded student solutions written in Python before. Half of the participants were graduate students and the other half were undergraduates.
...
To make the task more concrete, participants \DIFdelbegin \DIFdel{were given }\DIFdelend \DIFaddbegin \DIFadd{reviewed }\DIFaddend an example\footnote{Our example was drawn from the blog ``Practice Python: 30-minute weekly Python exercises for beginners,'' posted on Thursday, April 24, 2014, and titled ``SOLUTION Exercise 11: Check Primality and Functions.'' (\url{http://practicepython.blogspot.com})} of a class forum post that used examples taken from student solutions to explain different strategies for solving a Python problem. \DIFdelbegin \DIFdel{They were also given }\DIFdelend \DIFaddbegin \DIFadd{For reference, subjects had }\DIFaddend print-outs of the prompts for each of the three problems in our dataset\DIFdelbegin \DIFdel{, to reference when looking at solutions}\DIFdelend .

{\bf Training} \DIFdelbegin \DIFdel{Given the subjects ' extensive experience with web-browsers, }\DIFdelend \DIFaddbegin \DIFadd{The subjects already had extensive experience using web browsers, so }\DIFaddend training for the baseline interface was minimal. Prior to using the OverCode interface, subjects watched a 3-4 minute long training video demonstrating the features of OverCode, \DIFdelbegin \DIFdel{and were given an opportunity to become familiar }\DIFdelend \DIFaddbegin \DIFadd{familiarized themselves }\DIFaddend with the interface\DIFdelbegin \DIFdel{and ask }\DIFdelend \DIFaddbegin \DIFadd{, and asked }\DIFaddend questions. The training session focused on the problem that would not be used in the main tasks, in order to avoid learning effects.
...
A second 12-person study was designed, similar in structure to the forum post study, but focused on measuring the coverage achieved by subjects in a fixed amount of time (15 minutes) when browsing and producing feedback on a large number of student solutions. The \DIFdelbegin \DIFdel{second study's task }\DIFdelend \DIFaddbegin \DIFadd{task in the second study }\DIFaddend was more constrained than the first\DIFdelbegin \DIFdel{: instead }\DIFdelend \DIFaddbegin \DIFadd{. Instead }\DIFaddend of writing a freeform post, \DIFdelbegin \DIFdel{subjects were asked }\DIFdelend \DIFaddbegin \DIFadd{experimenters asked subjects }\DIFaddend to identify the five most frequent strategies used by students and rate their confidence that these strategies \DIFdelbegin \DIFdel{were frequently used }\DIFdelend \DIFaddbegin \DIFadd{occurred frequently }\DIFaddend in the student solutions. These changes to the task enabled us to measure coverage in terms of solutions read, the relevance of written feedback and the \DIFdelbegin \DIFdel{subject's perceived coverage . In this study , the rest of the hypotheses , i.e., }\DIFdelend \DIFaddbegin \DIFadd{perceived coverage of the subject. This study tests the remaining hypotheses on read coverage and speed (}{\bf \DIFaddend H2\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin }\DIFadd{), feedback coverage (}{\bf \DIFaddend H3\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin }\DIFadd{) and perceived coverage (}{\bf \DIFaddend H4\DIFdelbegin \DIFdel{, are tested}\DIFdelend \DIFaddbegin }\DIFadd{)}\DIFaddend .

\DIFdelbegin \DIFdel{Prior to running the second study, both the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend OverCode and baseline interfaces \DIFdelbegin \DIFdel{were slightly modified}\DIFdelend \DIFaddbegin \DIFadd{changed slightly prior to running the second study}\DIFaddend . Figure \ref{iterPowerEdXControl} shows the baseline changes that reduce the differences between the rendering of solutions in the baseline and OverCode interfaces. The OverCode interface was changed to show identifiers next to every stack and solutions so that subjects could provide it when asked.

\subsubsection{Participants, Apparatus, Conditions}

The coverage study shared the same methods for recruiting participants, apparatus and conditions as the forum post study. 12 new participants (11 male) participated in the second study (mean age = 25.4, $\sigma = 6.9$). Across those 12 participants, the mean years of Python programming experience was 4.9 ($\sigma = 3.0$) and 9 of them had previously graded code (5 had graded Python code). \DIFdelbegin \DIFdel{There were }\DIFdelend \DIFaddbegin \DIFadd{Subjects included }\DIFaddend 5 graduate students, 6 undergraduates, and 1 independent computer software professional.


\subsubsection{Procedure}
{\bf Prompt} In the coverage study, the prompt was similar to the one used in the forum post study, explaining that the subjects would be tackling the problem of producing feedback for students at scale. The language was modified to shift the focus towards finding frequent strategies used by students, rather than any example of good or bad code used by a student.\DIFdelbegin %DIFDELCMD < \todo{find prompt and put it in here}
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo{find prompt and put it in here}

{\bf Training} As before, subjects \DIFdelbegin \DIFdel{were shown }\DIFdelend \DIFaddbegin \DIFadd{watched }\DIFaddend a training video and given time to practice using OverCode features prior to their trial in the OverCode condition.

{\bf Task} The coverage study task consisted of a more constrained feedback task. Given 15 minutes with either the OverCode or baseline interface, subjects \DIFdelbegin \DIFdel{were asked to fill }\DIFdelend \DIFaddbegin \DIFadd{filled }\DIFaddend out a table, identifying the five most frequent strategies used by students to solve the problem. For each strategy they identified, they \DIFdelbegin \DIFdel{were asked to fill }\DIFdelend \DIFaddbegin \DIFadd{filled }\DIFaddend in the following fields in the table:
 \begin{itemize} 
...
\item Their confidence, on a scale of 1-7, that the strategy frequently occurred in the student solutions.
 \end{itemize} 
Importantly, subjects \DIFdelbegin \DIFdel{were also asked to mark }\DIFdelend \DIFaddbegin \DIFadd{marked }\DIFaddend solutions or stacks as \emph{read} by clicking on them after they had processed them, even if they \DIFdelbegin \DIFdel{were not choosing }\DIFdelend \DIFaddbegin \DIFadd{did not choose }\DIFaddend them as representative strategies. \DIFdelbegin \DIFdel{Combined with interaction logging done by the system, this enabled us to measure }\DIFdelend \DIFaddbegin \DIFadd{We combined this data with logs of subject actions in the interface to quantify }\DIFaddend read coverage.
...
{\bf H3: Feedback coverage} Each subject reported on the five most frequent strategies in a set of solutions, by copying both a code example and the identifier of the solution (baseline) or stack (OverCode) that it came from. We define \emph{feedback coverage} as the number of students for which the quoted code is relevant, in the sense that they wrote the same lines of code, ignoring differences in whitespace or variable names. We computed the coverage for each example using the following process:
 \begin{itemize} 
\item Reduce the quoted code down to only the lines referred to in the annotation. Often, the \DIFdelbegin \DIFdel{subject's }\DIFdelend annotation would focus on a specific feature of the quoted code, which sometimes \DIFdelbegin \DIFdel{had additional lines that were }\DIFdelend \DIFaddbegin \DIFadd{included additional lines }\DIFaddend unrelated to the \DIFdelbegin \DIFdel{subject's }\DIFdelend \DIFaddbegin \DIFadd{written }\DIFaddend feedback. For example, comments about iterating over a range function, while also quoting the contents of the for loop. This step meant we would be calculating the coverage of a more general\DIFdelbegin \DIFdel{(smaller ) }\DIFdelend \DIFaddbegin \DIFadd{, smaller }\DIFaddend set of lines.

\item Find the source stack that the quoted code comes from. This is trivial in the OverCode condition, where the \DIFdelbegin \DIFdel{stack ID is included in the subject's post}\DIFdelend \DIFaddbegin \DIFadd{subject wrote down a stack ID}\DIFaddend . In the baseline \DIFdelbegin \DIFdel{, we used the solution ID included in the subject's post to find the stack that it was merged into by the backend }\DIFdelend \DIFaddbegin \DIFadd{condition, the subject wrote down the solution ID. During post-study analysis, the authors determined which stack the solution is in, as determined by the OverCode analysis }\DIFaddend pipeline.
...
{\bf H4: Perceived coverage} After using each interface, we asked participants how strongly they agreed with the statement `This interface helped me develop a high-level view of students' understanding and misconceptions,' which quotes the first part of our third hypothesis. Participants agreed with this statement after using OverCode significantly more than when using the baseline interface ($W=63, Z=2.70, p<0.01, r=0.81$). Statistical significance was computed using the Wilcoxon Signed Rank test, pairing users' ratings of each interface. The analysis was done on \DIFaddbegin \DIFadd{the data of }\DIFaddend 11 participants \DIFdelbegin \DIFdel{' data , as one participant 's data }\DIFdelend \DIFaddbegin \DIFadd{because the data of one participant }\DIFaddend was lost. The mean rating with standard error for the responses is shown in Figure~\ref{perceivedCoverage}(a). 

For each strategy identified by subjects, we asked them to rate their confidence, on a scale of 1-7, that the strategy was frequently used by students in the dataset. Mean confidence ratings on a per-problem basis are shown in Figure \ref{perceivedCoverage}(b). We found that for \codevar{compDeriv}, subjects using OverCode \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{felt }\DIFaddend significantly more confident that their annotations were relevant to many students, compared to the baseline (Mann--Whitney $U=260.5, n_{1}=18, n_{2}=16, p<0.0001$).
...
Immediately after finishing the assigned tasks with an interface, participants rated their agreement with statements about the appropriateness of various adjectives to describe the interface they just used, on a 7-point Likert scale. While participants found the baseline to be significantly more simple ($W=2.5, Z=-2.60, p<0.01, r=0.78$), they found OverCode to be significantly more flexible ($W=45, Z=2.84, p<0.005, r=0.86$), less tedious ($W=3.5, Z=-2.41, p<0.05, r=0.73$), more interesting ($W=66, Z=2.96, p<0.001, r=0.89$), and more enjoyable ($W=45, Z=2.83, p<0.005, r=0.85$). The analysis was done on \DIFaddbegin \DIFadd{the data of }\DIFaddend 11 participants \DIFdelbegin \DIFdel{' data , as one participant 's data }\DIFdelend \DIFaddbegin \DIFadd{because the data of one participant }\DIFaddend was lost. The mean ratings (with standard error) for the responses are shown in Figure~\ref{studyLikert1_onPop2}. 

After the completion of the Coverage Study, participants \DIFdelbegin \DIFdel{were asked again to rate }\DIFdelend \DIFaddbegin \DIFadd{rated }\DIFaddend their agreement with statements about each interface on a 7-point Likert scale. After using both interfaces to view thousands of solutions, there were no significant differences between how overwhelming or easy to use each interface was. However, participants did feel that OverCode ``helped me get a sense of my students' understanding'' more than the baseline ($W=62.5, Z=-2.69, p<0.01, r=0.78$). The mean ratings \DIFdelbegin \DIFdel{(with standard error) }\DIFdelend for the responses are shown in Figure~\ref{studyLikert1_onPop2}\DIFaddbegin \DIFadd{, as well as the standard error}\DIFaddend .
...
{\bf Usage and Usefulness of Interface Features} In the second part of the post-study survey, participants rated their agreement with statements about the helpfulness of various interface features on a 7-point Likert scale. There were only two features to ask about in the baseline interface\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend in-browser find and viewing raw solutions\DIFdelbegin \DIFdel{), which }\DIFdelend \DIFaddbegin \DIFadd{. Statements about these baseline features }\DIFaddend were mixed in with statements about \DIFdelbegin \DIFdel{features in the OverCode interface}\DIFdelend \DIFaddbegin \DIFadd{OverCode features}\DIFaddend . The OverCode feature of stacking equivalent solutions was found more helpful than the baseline features of in-browser find ($W=41, Z=2.07, p<0.05, r=0.60$) and viewing raw student solutions, comments included ($W=45, Z=2.87, p<0.005, r=0.83$). \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Subjects found both the }\DIFaddend OverCode feature of variable renaming and previewing rewrite rules \DIFdelbegin \DIFdel{were also both found }\DIFdelend significantly more helpful than seeing raw student code ($W=65.5, Z=2.09, p<0.05, r=0.61$ and $W=56.5, Z=2.14, p<0.05, r=0.62$, respectively). The mean ratings for the features are shown in Figure~\ref{featureHelpfulness}.
...
\DIFdelbegin \DIFdel{Our }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend three-part evaluation of the OverCode backend and user interface demonstrates its usability and usefulness in a realistic teaching scenario. Given that \DIFdelbegin \DIFdel{we focused on the }\DIFdelend \DIFaddbegin \DIFadd{the datasets are drawn from the }\DIFaddend first three weeks of an introductory Python programming course, \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend evaluation is limited to single functions, whose most common solutions were less than ten lines long. Some of these functions were recursive, while most were iterative. Variables were generally limited to booleans, integers, strings, and lists of these basic data types. All solutions had already passed the autograder tests, and study participants still found solutions that suggested student misconceptions or knowledge gaps. Future work will address more complex algorithms and data types.

\subsubsection{Read coverage}
\DIFdelbegin \DIFdel{First, we observed that subjects were able to effectively read less in order to cover more ground }\DIFdelend \DIFaddbegin \DIFadd{Subjects covered more student solutions when reading solutions in the OverCode interface }\DIFaddend (H1, read coverage). This is expected, because in OverCode each read stack represented tens or hundreds of raw solutions, while there was only a 1-to-1 mapping between read solutions and raw solutions in the baseline condition. The OverCode backend produces platonic solutions that represent many raw solutions, reducing the cognitive load of mentally processing all the raw solutions, including their variation in formatting and variable naming.

Figure \ref{readCoverage}(a) shows that in some cases, subjects \DIFdelbegin \DIFdel{were able to }\DIFdelend read nearly as many (\codevar{hangman}) or more (\codevar{iterPower}) function definitions in the baseline as in OverCode. In the case of \codevar{iterPower}, the raw solutions are repetitive because of the simplicity of the problem and the relatively small amounts of variation demonstrated by student solutions. This can explain the \DIFdelbegin \DIFdel{subjects' ability }\DIFdelend \DIFaddbegin \DIFadd{ability of subjects }\DIFaddend to move quickly through the set of solutions, reading as many as 90 solutions in 15 minutes.
...
Stacey Terman, in partial fulfilment of her Master's of Engineering, worked with me to extend OverCode to a new domain, i.e., incorrect solutions, and a new task, i.e., grading hundreds of incorrect solutions by hand\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{staceythesis}}%DIFAUXCMD
...
The GroverCode interface, shown in Figure~\ref{fig:whole_interface}, has several additions and distinctions from the OverCode interface. As shown in Figure~\ref{fig:correct_and_incorrect_stacks}, both correct solutions and incorrect solutions are displayed, differentiable by their varied background colors and red X \DIFdelbegin \DIFdel{'s }\DIFdelend next to every failed test. To better explain each test failure, the \DIFdelbegin \DIFdel{solution's differing }\DIFdelend actual and expected outputs are \DIFdelbegin \DIFdel{also displayed . A rubric , shown in }\DIFdelend \DIFaddbegin \DIFadd{displayed beneath each solution. The staff iteratively develop a rubric as they grade. }\DIFaddend Figure~\ref{fig:rubric} \DIFdelbegin \DIFdel{, is iteratively developed by the grading staff as they grade}\DIFdelend \DIFaddbegin \DIFadd{is a snapshot of one of these rubrics}\DIFaddend .
...
Like OverCode, GroverCode groups correct solutions into stacks. Grades are propagated to all the solutions in a stack. \DIFdelbegin \DIFdel{Incorrect solutions were not also grouped }\DIFdelend \DIFaddbegin \DIFadd{OverCode does not group incorrect solutions }\DIFaddend into stacks because the stacking process \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{for incorrect solutions is too }\DIFaddend new and experimental for field deployment in an actual grading session. GroverCode still normalized all solutions, both correct and incorrect.
...
In OverCode, a single-line difference between two solutions could \DIFdelbegin \DIFdel{, by affecting a variable 's behavior , cause }\DIFdelend \DIFaddbegin \DIFadd{that affects variable behavior causes }\DIFaddend the variable renaming process to propagate \DIFdelbegin \DIFdel{that difference as a variable name change }\DIFdelend \DIFaddbegin \DIFadd{naming differences }\DIFaddend throughout all the lines that mention the affected variable. This non-locality of difference \DIFdelbegin \DIFdel{made }\DIFdelend \DIFaddbegin \DIFadd{makes }\DIFaddend it harder to spot \DIFdelbegin \DIFdel{in the user interface where the actual }\DIFdelend \DIFaddbegin \DIFadd{where the original }\DIFaddend syntactic difference between two platonic solutions \DIFdelbegin \DIFdel{was}\DIFdelend \DIFaddbegin \DIFadd{is}\DIFaddend . As a result, \DIFdelbegin \DIFdel{stacks are rendered }\DIFdelend \DIFaddbegin \DIFadd{the rendering of stacks }\DIFaddend in the GroverCode user interface \DIFdelbegin \DIFdel{slightly differently}\DIFdelend \DIFaddbegin \DIFadd{is slightly different than in OverCode}\DIFaddend . Even if the normalized variable names are different between two syntactically identical lines of code in two different stacks, if the variables take on the same sequence of values {\it just within that line of code}, it will not be highlighted as different in the user interface. Only the line with the \DIFaddbegin \DIFadd{original }\DIFaddend syntactic difference will be highlighted.
...
\item an ordered list of the sequences of values each blanked-out variable in the line took on during execution, e.g., the first blank in \texttt{for \underline{\hspace{1em}} in range(\underline{\hspace{1em}}):} might iterate through \texttt{1,2,3} while, with each visitation of this line during execution, the second blank stays constant at \texttt{[1,2,3],[1,2,3],[1,2,3]}
 \end{enumerate} 
\DIFaddbegin \DIFadd{Item 1 in this list, the template, captures the syntax of the line, ignoring the variables mentioned in it. }\DIFaddend Items 1 and 2 in this list together \DIFdelbegin \DIFdel{contain the information in }\DIFdelend \DIFaddbegin \DIFadd{capture the information necessary to reconstruct }\DIFaddend the original normalized lines in OverCode\DIFdelbegin \DIFdel{and information about the line's syntax, regardless of the variables identities}\DIFdelend . Items 1 and 3 capture the information necessary to tell what the variable behavior is just within that one line and whether two lines in two different stacks will be highlighted as different from each other. This information is also used for variable renaming in incorrect solutions.
...
Approximately two hundred students \DIFdelbegin \DIFdel{were }\DIFdelend enrolled in 6.0001 \DIFaddbegin \DIFadd{during the spring 2016 semester}\DIFaddend , and nine instructors used GroverCode to grade nearly all student solutions. %Seven programming problems of a variety of difficulties were graded over the course of these two sessions.
...
Using the GroverCode user interface, nine instructors, including one lecturer and eight teaching assistants (TAs) graded these solutions as part of their official grading responsibilities. Stacey Terman, the Master's of Engineering student who implemented most of GroverCode, was one of these TAs. \DIFdelbegin \DIFdel{Solutions that did not successfully pass through the pipeline were graded by hand afterwards. }\DIFdelend \DIFaddbegin \DIFadd{GroverCode logged }\DIFaddend TA grading events, \DIFdelbegin \DIFdel{e.g., }\DIFdelend \DIFaddbegin \DIFadd{such as }\DIFaddend adding or applying rubric items and point values to solutions\DIFdelbegin \DIFdel{, were logged. }\DIFdelend \DIFaddbegin \DIFadd{. TAs hand-graded solutions that did not successfully pass through the GroverCode pipeline. }\DIFaddend The observer (the author of this thesis) took extensive notes during each day-long grading session to capture spontaneous feature requests as well as bugs and complaints. 
